...
const getItemList = () => {
  ...
  if (searchValue.value.trim()) {
    filter = `assetName like %${searchValue.value}%`
  }
...
const submitCreateForm = async () => {
  try {
    // 基础验证
    if (!createForm.supplierId || !createForm.assetType) {
      return ElMessage.error('请选择供应商和资产类型')
    }
    ...
    const payload = {
      ...
      assetName: createForm.assetAttributes.name ? createForm.assetAttributes.name : createForm.assetAttributes.title,
      quantity: createForm.quantity,
      unitPrice: createForm.unitPrice,
      currency: createForm.currency,
      totalPrice: createForm.quantity * createForm.unitPrice,
      assetAttributes: createForm.assetAttributes
    }

    // 类型相关验证
    switch (createForm.assetType) {
      case 'BOOK':
        if (!createForm.assetAttributes.isbn || !createForm.assetAttributes.title) {
          return ElMessage.error('请填写图书的必填信息')
        }
        break
      case 'CONSUMABLE':
        if (!createForm.assetAttributes.name || !createForm.assetAttributes.unit) {
          return ElMessage.error('请填写耗材的必填信息')
        }
        break
      case 'EQUIPMENT':
        if (!createForm.assetAttributes.name || !createForm.assetAttributes.serialNumber) {
          return ElMessage.error('请填写设备的必填信息')
        }
        break
    }

    console.log(payload)
    await http.$post('/purchaseOrder', payload)
  ...
  <div class="page-container">
    ...
    >
      <el-form label-width="100px">
        ...
        <!-- 动态表单区域 -->
        <template v-if="createForm.assetType === 'BOOK'">
          <el-form-item label="ISBN" required>
            <el-input v-model="createForm.assetAttributes.isbn" />
          </el-form-item>
          <el-form-item label="书名" required>
            <el-input v-model="createForm.assetAttributes.title" />
          </el-form-item>
          <el-form-item label="作者">
            <el-input v-model="createForm.assetAttributes.author" />
          </el-form-item>
          <el-form-item label="出版社">
            <el-input v-model="createForm.assetAttributes.publisher" />
          </el-form-item>
        </template>

        <template v-else-if="createForm.assetType === 'CONSUMABLE'">
          <el-form-item label="名称" required>
            <el-input v-model="createForm.assetAttributes.name" />
          </el-form-item>
          <el-form-item label="类型">
            <el-input v-model="createForm.assetAttributes.type" />
          </el-form-item>
          <el-form-item label="单位" required>
            <el-input v-model="createForm.assetAttributes.unit" />
          </el-form-item>
        </template>

        <template v-else-if="createForm.assetType === 'EQUIPMENT'">
          <el-form-item label="名称" required>
            <el-input v-model="createForm.assetAttributes.name" />
          </el-form-item>
          <el-form-item label="序列号" required>
            <el-input v-model="createForm.assetAttributes.serialNumber" />
          </el-form-item>
          <el-form-item label="状态">
            <el-select v-model="createForm.assetAttributes.status">
